...
#define LEARNING_RATE 0.01f //Changed
#define REPLAY_MEMORY 20000
#define BATCH_SIZE 512
#define USE_LSTM true //Changed
...
#define REWARD_WIN  0.1f //Changed
#define REWARD_LOSS -0.1f //Changed
...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	*/
	...
	//gazebo::transport::SubscriberPtr cameraSub = cameraNode->Subscribe("/gazebo/arm_world/camera/link/camera/image_name", &ArmPlugin::onCameraMsg, this);

	
	cameraSub = cameraNode->Subscribe("/gazebo/arm_world/camera/link/camera/image_name", &gazebo::ArmPlugin::onCameraMsg, this);;
...
bool ArmPlugin::createAgent()
{
	...
	*/
	...
	agent = NULL;
	uint32_t numActions = INPUT_CHANNELS *2;
    agent = dqnAgent::Create(INPUT_WIDTH, INPUT_HEIGHT, 
                       INPUT_CHANNELS, numActions, OPTIMIZER, 
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		if (collisionCheck)
		{
		    ...
		    // If gripper is the collision point .. reward win
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		*/
		...
		if(DEBUG){printf("Gripper bbox x(%f,%f) y(%f,%f) z(%f,%f)\n",
			gripBBox.min.x, gripBBox.max.x,
			gripBBox.min.y, gripBBox.max.y,
